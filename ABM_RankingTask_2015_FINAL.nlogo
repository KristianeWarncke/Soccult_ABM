...
;;fixed var
  ...
  neuroticism               ;;Get angry for no particular reason (actively interrupting a debate round)
...
to setup-globals
  set agent_names ["a" "b" "c" "d" "e" "f" "g" "h" "i" "j"]
...
to setup-people
  crt team_size
    [
      ...
      set color green
      set change? true
      set-traits         ;; also sets score
      show-ranking ranking score
      show-traits agreeableness talkativeness neuroticism critical_thinking knowledge
...
to print-headers-to-file
  ...
  ask turtles [
      file-print (word "Agent " [item who agent_names] of self
        " A: " agreeableness " T: " talkativeness " N: " neuroticism " C: " critical_thinking " K: " knowledge " initial list: " ranking)
  ]
  file-print ""
  file-close
end
  ...
  ask turtles
    [
      ...
      [
        ...
        ask other turtles
          [
            let neuro_var random 4 + 1 ; gives a number between 1 and 4
            ifelse (stoch-step [neuroticism] of self and neuro_var > 2) ; if neuro, then only interrupt half of the time
            [ ;If one is neuro then the round is interrupted and the others do not check
            ]
            [ ;If no one is neuro, the round continues as usual
...
to set-traits  ;; turtle procedure
    ;; Initialize all traits to 1
  set agreeableness 1
  set talkativeness 1
  set neuroticism 1
  set critical_thinking 0

  set knowledge 0.2

  ;; Randomly choose one trait to assign a value from a normal distribution
  let traits ["agreeableness" "talkativeness" "neuroticism" "critical_thinking" "knowledge"]
  let chosen_trait one-of traits

    ;; Assign the chosen trait using a normal distribution
  if chosen_trait = "agreeableness" [
    let temp_agree 0
    set agent_names replace-item who agent_names "agree_agent"
    while [temp_agree <= mu_agree] [
      set temp_agree precision (random-normal mu_agree sd_agree) 2 ]
    set agreeableness temp_agree]           ;;willingness to change

  if chosen_trait = "talkativeness" [
    let temp_talk 0
    set agent_names replace-item who agent_names "talk_agent"
    while [temp_talk <= mu_talk] [
      set temp_talk precision (random-normal mu_talk sd_talk) 2 ]
    set talkativeness temp_talk]           ;;willingness to change

  if chosen_trait = "neuroticism" [
    let temp_neuro 0
    set agent_names replace-item who agent_names "neuro_agent"
    while [temp_neuro <= mu_neuro] [
      set temp_neuro precision (random-normal mu_neuro sd_neuro) 2 ]           ;;
    set neuroticism temp_neuro]

  ;set communication_skill precision (random-normal mu_comm sd_comm) 2     ;;ability to communicate

  if chosen_trait = "critical_thinking" [
    let temp_crit 0
    set agent_names replace-item who agent_names "crit_agent"
    while [temp_crit <= 0.2] [
      set temp_crit precision (random-normal mu_crit sd_crit) 2 ]      ;;ability to evaluate others' arguments
    set critical_thinking temp_crit]

  if chosen_trait = "knowledge" [
    let temp_know 0
    set agent_names replace-item who agent_names "know_agent"
    while [temp_know <= 0.5 or temp_know > 1] [
      set temp_know precision (random-normal mu_know sd_know) 2 ]      ;;ability to evaluate others' arguments
    set knowledge temp_know]


  ; defining each agent's starting ranking list depending on their knowledge score
  let list_size length rankings
  let index 0
  let temp_mu (list_size - ((list_size / 10) * (knowledge * 10)))
  let temp_sd (list_size / 10)
  while [index <= 0 or index > list_size] [
    set index floor precision (random-normal temp_mu temp_sd) 2]
  ; Select the ranking and corresponding score based on the calculated index
  set ranking item index rankings
...
to show-traits [w x y z q]
      ...
      ask patch-at-heading-and-distance 90 10 [ set plabel q]
end
...
<experiments>
  ...
  <experiment name="own_abm" repetitions="10000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>team_score</metric>
    <metric>gain_score</metric>
    <metric>synergy_score</metric>
    <metric>agent_names</metric>
    <metric>reached_consensus?</metric>
    <metric>[agreeableness] of turtles</metric>
    <metric>[talkativeness] of turtles</metric>
    <metric>[neuroticism] of turtles</metric>
    <metric>[critical_thinking] of turtles</metric>
    <metric>[knowledge] of turtles</metric>
    <enumeratedValueSet variable="team_size">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="game_size">
    ...
    <enumeratedValueSet variable="knowledge_sharing">
      <value value="0.8"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
