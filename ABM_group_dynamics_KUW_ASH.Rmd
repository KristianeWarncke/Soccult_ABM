...
dominating_traits1 <- group_data_wide %>%
  mutate(
...
all_agreeableness_values <- unlist(dominating_traits1$agreeableness_dominant)
...
sd_average_steps_per_diversity <- average_steps_per_diversity %>%
  group_by(diversity_level) %>%
  summarise(sd_avg_steps_diversity = std.error(avg_steps_diversity, na.rm = TRUE), .groups = 'drop')

# Join average steps back to the original dataframe
group_data_wide <- average_steps_per_diversity %>%
  left_join(avg_steps_diversity, by = "diversity_level")

# Join standard deviation of steps back to the original dataframe
group_data_wide <- sd_average_steps_per_diversity %>%
  left_join(sd_average_steps_diversity, by = "diversity_level")
...
```{r}
#team score and average traits
team_traits_plot <- ggplot(data = averaged_traits, aes(x = team_score , y = avg_agreeableness)) + geom_point() 
  #stat_summary(fun = mean, geom = "point", shape = 18, size=2, colour="white", alpha = 0.8)+ 
  guides(fill=FALSE)+
  #scale_fill_manual(name = "Averaged Traits", palette = "Paired")+
  labs(title = "Team Score across Diversity Level", y = "Average Team Score", x = "Averaged Traits")+
  #scale_x_discrete(labels=  c("Low", "Medium", "High")) + 
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5, size = 16))
...
averaged_traits <- group_data_wide %>%
  mutate(avg_neuroticism = rowMeans(select(., starts_with("agents_neuroticism")), na.rm = TRUE),
    avg_agreeableness = rowMeans(select(., starts_with("agents_agreeableness")), na.rm = TRUE),
    avg_talkativeness = rowMeans(select(., starts_with("agents_talkativeness")), na.rm = TRUE),
    avg_critical = rowMeans(select(., starts_with("agents_critical")), na.rm = TRUE),
    avg_knowledge = rowMeans(select(., starts_with("agents_knowledge")), na.rm = TRUE),
  ...
  select(avg_neuroticism, avg_agreeableness, avg_talkativeness, avg_critical, avg_knowledge, team_score)
