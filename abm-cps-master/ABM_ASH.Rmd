...
m1 <- lm(team_score ~ diversity_level, data = df)
summary(m1)

# synergy model
m2 <- lm(synergy_score ~ diversity_level, data = df)
summary(m2)

# gain score model
m3 <- lm(gain_score ~ diversity_level, data = df)
summary(m3)
```

H1.2: diversity, n_steps
```{r}
m4 <- lm(n_steps ~ diversity_level, data = df)
summary(m4)
```
...
LAV OM TIL STANDARDIZED


```{r}
m5 <- lm(n_steps ~ agree_dom_std + talk_dom_std + crit_dom_std + know_dom_std + neuro_dom_std, data = df)
summary(m5)
```
...
```{r}
m6 <- lm(gain_score ~ agree_dom_std + talk_dom_std + crit_dom_std + know_dom_std + neuro_dom_std, data = df)
summary(m6)

m7 <- lm(synergy_score ~ agree_dom_std + talk_dom_std + crit_dom_std + know_dom_std + neuro_dom_std, data = df)
summary(m7)
```
```
